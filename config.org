...
#+begin_src emacs-lisp
(defun nl/indent-line-function()
              (lambda ()
                (let ((col (* 3 (car (syntax-ppss)))))
                  (indent-line-to col))))
...
(defun nl/templ-mode-hook()
  ...
  (setq tab-width 3
        standard-indent 3
        indent-tabs-mode nil)
  (setq-local indent-line-function `nl/indent-line-function)
  (subword-mode +1)
  )
...
(defun nl/tsx-comment-dwim (arg)
  "Toggle JSX comment {/* */} at the beginning of each line in tsx-mode."
  (interactive "P")
  (let ((beg (if (use-region-p) (region-beginning) (line-beginning-position)))
        (end (if (use-region-p) (region-end) (line-end-position))))
    (save-excursion
      (goto-char beg)
      (if (looking-at "[ \t]*{/\\*")  ;; Detect already commented JSX lines
          ;; Uncomment each line
          (while (<= (point) end)
            (beginning-of-line)
            (when (looking-at "[ \t]*{/\\* ?")
              (delete-region (match-beginning 0) (+ (match-beginning 0) 4)))
            (end-of-line)

...
(use-package emacs
  ...
  (defun nl/set-fonts (frame)
    ...
    (let ((fixed-font-name (format "%s %s" nl/gui-fixed-font-name nl/gui-fixed-font-size-normal))
      ...
      (set-face-font 'default fixed-font-name)
      ...
      (set-face-attribute 'mode-line-buffer-id nil :family nl/gui-variable-font-name :foreground "goldenrod" :weight 'bold)
...
Allow changing themes on the fly with this.
...
(use-package lsp-ui
  ...
  ;;   (set-face-attribute 'default frame :family "Hack" :height 180))
  :hook
...
The Perspective package provides multiple named workspaces (or "perspectives") in Emacs, similar to multiple
desktops in window managers like Awesome and XMonad, and Spaces on the Mac.

...
(defun nl/set-favorite-font-size (size)
  ...
  (let ((fav-font (format "%s-%d" nl/gui-fixed-font-name size)))
    ...
    (set-face-attribute 'variable-pitch nil :family nl/gui-variable-font-name :inherit 'default :height 150)))
...
(use-package ef-themes
  ...
  (nl/fix-gnus-group-news-face-cycle))
#+end_src
...
(unless (string-match "-microsoft" (downcase operating-system-release))
  (use-package xresources-theme
    :load-path "~/src/github/elisp/xresources-theme"
    :init
    (add-to-list 'custom-theme-load-path "~/src/github/elisp/xresources-theme")
...
#+begin_src emacs-lisp :tangle no
(let ((theme-path "~/src/github/elisp/doom-two-tone-themes/themes/"))
  (when (file-directory-p (expand-file-name theme-path))
...
(defun nl/consult-ripgrep-project-src ()
  ...
  (let* ((project-root (projectile-project-root)))
    (if (not project-root)
      ...
      (let* ((src-path (expand-file-name nl/project-src-folder project-root))
             (test-path (expand-file-name nl/project-test-folder project-root))
             (globs (delq nil
                          (list (when (file-directory-p src-path)
                                  (format " --glob %s/**" nl/project-src-folder))
                                (when (file-directory-p test-path)
                                  (format " --glob %s/**" nl/project-test-folder))))))
        (if (null globs)
            (message "No src/test folder in project.")
          (let ((consult-ripgrep-args (concat consult-ripgrep-args (apply #'concat globs))))
...
(defun nl/typescript-mode ()
  ...
  (column-enforce-mode +1)
  (subword-mode +1)
  ;; Ensure the TS/TSX parser is used when running Prettier on save.
  (setq-local prettier-parsers '(typescript babel-ts))
  (prettier-mode +1)
  ;;(eldoc-mode +1)
  ;;(company-mode +1)
...
(use-package typescript-ts-mode
  :mode (("\\.ts\\'" . typescript-ts-mode)
         ...
         ("\\.jsx\\'" . tsx-ts-mode))
  :hook
  ((typescript-ts-mode . my/lsp-deferred-unless-remote)
   (typescript-ts-mode . nl/typescript-mode)
   (tsx-ts-mode . my/lsp-deferred-unless-remote)
...
(use-package org-modern
  ...
  (org-modern-hide-stars t) ;; hide stars in headlines

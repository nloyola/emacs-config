...
(use-package emacs
  ...
  (defun my/consult-projectile-compile-with-history ()
    "Prompt for a compile command with Consult, then run it with Ghostel."
    (interactive)
    (require 'projectile)
    (require 'consult)
    ...
    (let* ((project-root (projectile-project-root))
           ...
           (command (consult--read
                     ...
                     :prompt "Ghostel compile command: "
                     :default default)))
...
(add-to-list 'display-buffer-alist
             '("\\*\\(?:compilation\\|ghostel-compile\\|Backtrace\\|Warnings\\|Completions\\|Compile-Log\\|cargo-run\\|\\Flycheck.*\\|Go Test\\)\\*"
               display-buffer-in-side-window
               (side . bottom)
               (slot . 1)
               (preserve-size . (nil . nil))

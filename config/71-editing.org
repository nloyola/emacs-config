...
(use-package hydra
  ...
  (defun nl/open-current-folder-in-nautilus ()
    "Open the current buffer's folder in Nautilus."
    (interactive)
    (let ((folder (expand-file-name (nl/current-buffer-folder))))
      (when (file-remote-p folder)
        (user-error "Cannot open remote folder in Nautilus: %s" folder))
      (unless (executable-find "nautilus")
        (user-error "Nautilus executable not found"))
      (start-process "nautilus" nil "nautilus" folder)
      (message "Opened folder in Nautilus: %s" folder)))
  ...
  (defhydra hydra-nl-common (:color red :hint nil)
    ...
    ("o" nl/open-current-folder-in-nautilus "nautilus here" :color blue :column "External")

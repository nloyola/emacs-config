...
A package that uses the same idea from ace-jump-mode for buffer navigation, but applies it to
windows. The default keys are 1-9, but it's faster to access the keys on the home row, so that's
...
Expand region increases the selected region by semantic units. Just keep pressing the key until it
...
Simple Org-like code folding with built-in =hideshow=, based on
[[https://karthinks.com/software/simple-folding-with-hideshow/][Karthinks' hideshow cycling]].
...
(use-package hideshow
  ...
  :bind (:map hs-minor-mode-map
              ("C-<tab>" . nl/hs-cycle)
              ("C-S-<tab>" . nl/hs-global-cycle)
              ("C-<iso-lefttab>" . nl/hs-global-cycle))
  :preface
  (defun nl/hs-cycle (&optional level)
    "Cycle the fold at point like Org's TAB cycling.
With prefix LEVEL, hide blocks below that level."
    (interactive "p")
    (let (message-log-max
          (inhibit-message t))
      (if (= level 1)
          (pcase last-command
            ('nl/hs-cycle
             (hs-hide-level 1)
             (setq this-command 'nl/hs-cycle-children))
            ('nl/hs-cycle-children
             ;; `hs-show-block' needs to be called twice to open all folds of
             ;; the parent block.
             (save-excursion (hs-show-block))
             (hs-show-block)
             (setq this-command 'nl/hs-cycle-subtree))
            ('nl/hs-cycle-subtree
             (hs-hide-block))
            (_
             (if (not (hs-already-hidden-p))
               ...
               (setq this-command 'nl/hs-cycle-children))))
        (hs-hide-level level)
        (setq this-command 'nl/hs-hide-level))))

  (defun nl/hs-global-cycle ()
    "Cycle folding for the whole buffer."
    (interactive)
    (pcase last-command
      ('nl/hs-global-cycle
       (save-excursion (hs-show-all))
       (setq this-command 'nl/hs-global-show))
      (_ (hs-hide-all))))
  :config
...
(use-package hydra
  ...
  (defhydra hydra-nl-search (:hint nil)
    ...
    ("t" (nl/consult-ripgrep-project-src-test) "project's src/test folders" :exit t))
  ...
  (defun nl/copy-buffer-file-name ()
    "Copy the current buffer file name to the kill ring and clipboard."
    (interactive)
  ...
  (defun nl/current-buffer-folder ()
    "Return the folder associated with the current buffer."
    (cond
     ((derived-mode-p 'dired-mode) (dired-current-directory))
     (buffer-file-name (file-name-directory buffer-file-name))
     (t default-directory)))

  (defun nl/open-current-folder-in-thunar ()
    "Open the current buffer's folder in Thunar."
    (interactive)
    (let ((folder (expand-file-name (nl/current-buffer-folder))))
      (when (file-remote-p folder)
        (user-error "Cannot open remote folder in Thunar: %s" folder))
      (unless (executable-find "thunar")
        (user-error "Thunar executable not found"))
      (start-process "thunar" nil "thunar" folder)
      (message "Opened folder in Thunar: %s" folder)))
  ...
  (defhydra hydra-nl-common (:color red :hint nil)
    ...
    ("y" nl/copy-buffer-file-name "copy file path" :color blue :column "Copy")
    ("o" nl/open-current-folder-in-thunar "thunar here" :color blue :column "External")
    ("c" nl/open-alacritty-project-root "alacritty project root" :color blue)
    ("C" nl/open-alacritty-here "alacritty here" :color blue)
    ("v" nl/open-ghostel-project-root "ghostel project root" :color blue)
    ("V" nl/open-ghostel-here "ghostel here" :color blue)
...
Provides the =selected-minor-mode= for Emacs. When =selected-minor-mode= is active, the keybindings in
=selected-keymap= will be enabled when the region is active.
...
Substitute is a set of commands that perform text replacement (i) throughout the buffer, (ii)
limited to the current definition (per narrow-to-defun), (iii) from point to the end of the buffer,
and (iv) from point to the beginning of the buffer.

...
(defun nl/go-test-function-run ()
  ...
  (let* ((root (projectile-project-root))
    ...
    (nl/go-command-in-proj-root
     (format "go test ./%s -run ^%s$ -v" relpath testname))))
...
(define-key go-ts-mode-map (kbd "C-c , f") 'nl/go-test-function-run)
(define-key go-ts-mode-map (kbd "C-c , p") 'nl/go-test-package-run)
